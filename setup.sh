...
REPO="https://raw.githubusercontent.com/AuraFusion19/os/main/"
function CEKIP () {
MYIP=$(curl -sS ipv4.icanhazip.com)
IPVPS=$(curl -sS https://raw.githubusercontent.com/AuraFusion19/izinvps/main/ipx | grep $MYIP | awk '{print $4}')
if [[ $MYIP == $IPVPS ]]; then
domain
Pasang
else
...
echo -e "$green                $NC"                                                                                                                          
echo -e "$green♥ TERIMAKSIH TELAH MEMAKAI SCRIPT XHAMSTORE ♥$NC"
sleep 2
sleep 5
echo -e  "${green}┌──────────────────────────────────────────┐${NC}"
...
echo "XHAM STORE" > /etc/xray/username
echo ""
clear
author=$name
echo ""
...
function key2(){
    ...
    if [[ $key == "1" ]]; then
        ...
        curl -s https://raw.githubusercontent.com/AuraFusion19/os/main/config/token.txt > /etc/github/api
        curl -s https://raw.githubusercontent.com/AuraFusion19/os/main/config/email.txt > /etc/github/email
        curl -s https://raw.githubusercontent.com/AuraFusion19/os/main/config/nama.txt > /etc/github/username
        clear
        ...
        cd
        git clone https://github.com/AuraFusion19/izinvps >/dev/null 2>&1
		cd izinvps
...
clear
echo -e  "${green}┌──────────────────────────────────────────┐${NC}"
echo -e  "${green}│  \033[1;37mContoh subdomain ( myham )                    ${green}│${NC}"
echo -e  "${green}│    \033[1;37mxxx.hamstore.biz.id jadi subdomain kamu               ${green}│${NC}"
echo -e  "${green}└──────────────────────────────────────────┘${NC}"
echo " "
...
TIMES="10"
CHATID="6717101864"
KEY="8354831201:AAGswodkssHVMGR3RqVH4MjkuBOFPaHaIrk"
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/AuraFusion19/izinvps/main/ipx | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
...
"'&reply_markup={"inline_keyboard":[[{"text":"🔥ᴏʀᴅᴇʀ","url":"https://t.me/XHAMstore"},{"text":"🔥GRUP","url":"https://t.me/hstunnel"}]]}'
